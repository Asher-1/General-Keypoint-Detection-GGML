...
#if defined(GKD_USE_CUDA)
#include "ggml-cuda.h"
#endif
#if defined(GKD_USE_METAL)
#include "ggml-metal.h"
#endif
#if defined(GKD_USE_VULKAN)
...
static ggml_backend_t try_init_gpu_backend() {
    ...
    if (n > 0) {
        ...
        if (b) {
            GKD_LOG_INFO("GPU backend: CUDA device 0 (%d available)", n);
            return b;
        }
        GKD_LOG_WARN("ggml_backend_cuda_init(0) failed; using CPU");
    }
    return nullptr;
#elif defined(GKD_USE_METAL)
    ggml_backend_t b = ggml_backend_metal_init();
    if (b) {
        GKD_LOG_INFO("GPU backend: Metal");
        return b;
    }
    GKD_LOG_WARN("ggml_backend_metal_init failed; using CPU");
    return nullptr;
#elif defined(GKD_USE_VULKAN)
    int n = ggml_backend_vk_get_device_count();
    if (n > 0) {
        ggml_backend_t b = ggml_backend_vk_init(0);
        if (b) {
            GKD_LOG_INFO("GPU backend: Vulkan device 0 (%d available)", n);
            return b;
        }
        GKD_LOG_WARN("ggml_backend_vk_init(0) failed; using CPU");
    }
    return nullptr;
#else
...
BackendCtx init_backend_ctx(int n_threads, size_t graph_nodes) {
    ...
    BackendCtx ctx{};
    if (n_threads <= 0) {
        // The backend and the persistent threadpool below MUST agree on the
        // thread count: ggml_graph_plan takes n_threads from the backend while
        // compute indexes threadpool->workers[omp_thread].
    ...
    if (!ctx.cpu) {
        GKD_LOG_ERROR("ggml_backend_cpu_init returned null");
        return ctx;
    }
    ggml_backend_cpu_set_n_threads(ctx.cpu, n_threads);

    // Attach a persistent threadpool - amortizes the disposable-threadpool
    // setup ggml would otherwise pay per call.
    {
        ...
        if (ctx.threadpool) {
            ...
            GKD_LOG_WARN("ggml_threadpool_new failed; per-call threadpool");
        }
    }
    ...
    if (ctx.gpu) {
        ...
        if (!ctx.sched) {
            GKD_LOG_WARN("ggml_backend_sched_new failed; CPU-only");
...
bool backend_graph_alloc(BackendCtx& ctx, ggml_cgraph* graph, ggml_gallocr_t& galloc) {
    ...
    // CPU path: one gallocr per graph.
    if (!galloc) {
        galloc = ggml_gallocr_new(ggml_backend_get_default_buffer_type(ctx.cpu));
        if (!galloc) {
